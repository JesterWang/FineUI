...
namespace FineUI
{
    ...
    public class Grid : CollapsablePanel, IPostBackDataHandler, IPostBackEventHandler
    {
        ...
        protected override void OnFirstPreRender()
        {
            ...
            // 修正 extjs v6.0 引入的问题
            if (!ShowHeader && !ShowGridHeader && ShowBorder)
            {
                OB.AddProperty("bodyCls", "x-panel-default-outer-border-trbl");
            }
            ...
            {
                cls += String.Format(" f-grid-row-align-{0}", VerticalAlignName.GetName(RowVerticalAlign));
            }

            //if (AllowCellEditing)
            //{
            //    cls += " f-grid-cellediting";
            //}
        ...
        private string GetGridStore()
        {
            ...
            string postbackScript = GetPostBackEventReference("#SORT#").Replace("'#SORT#'", "'Sort$'+sorter.getProperty()+'$'+sorter.getDirection()");
            postbackScript = "var sorter=operation.getSorters()[0];if(sorter){" + postbackScript + "}return false;";
                ...
                if (!String.IsNullOrEmpty(editorDataStr))
                {
                    ...
                    foreach (JObject modifiedItem in _modifiedData)
                    {
                        ...
                        foreach (JProperty propertyObj in rowModifiedData.Properties())
                        {
                            ...
                            // v6.0.0 - 回发时不会改变行的Values和DataKeys属性，如果数据变化需要重新绑定
